# alter and new function need to be done in version 2.2.3:
# add run route line function                                                                                           #Done
# add velocity adjustment                                                                                               #Done
# add emergency stop button                                                                                             #Done
# refine ran route time limit                                                                                           #Done
...
# define a StopError Exception for emergency stop
class StopError(Exception):

        def __init__(self):
            super(StopError,self).__init__(self, "Emergency Stop")
...
class App(wx.App):
    ...
    def MortorInit(self):
        ...
        try:
            # set the work coordinate to default
            self.gcs.DPA(password='SKS')
...
class MainFrame(wx.Frame):
    ...
    def OnClose(self, event):
        ...
        self.mainCameraPanel.StopRun(None)
        ...
        if wx.MessageDialog.ShowModal(
            ...
            self.mainCameraPanel.hexpodPositionThreadFlag = False
            # set the work coordinate to default
            self.gcs.DPA(password='SKS')
...
class MainCameraPanel(CameraPanel):
    ...
    def __init__(self, camera, gcs, laser=None, username=None, *args, **kwargs):
        ...
        try:
            ...
            self.workCoordinate = pickle.load(file)
            file.close()
        except FileNotFoundError:
            if wx.MessageDialog.ShowModal(
                    wx.MessageDialog(parent=None,
                ...
                self.rotator = 0
                self.pos = [0, 0]
                self.workCoordinate = 0
        finally:
            # define and enable work coordinate
            self.gcs.KSD(csname=self.username, axes='W', values=self.workCoordinate)
            self.gcs.KEN(self.username)
        ...
        self.runButton = FunctionToggleButtons(parent=self, name=".\\icon\\run.bmp")
        self.stopButton = FunctionButtons(parent=self, name=".\\icon\\stop.bmp")
        ...
        # Main camera panel has a run route line dialog
        self.runRoutLineDialog = self.RunRouteLineDialog(self)
        ...
        self.Sizer.AddMany(
            [(self.queryPostionWindow, 1, wx.ALIGN_LEFT | wx.TOP, 2), (1, 0, 1, 0), (self.homeButton, self.sizerFlag),
             ...
             (self.zigZagButton, self.sizerFlag), (self.circleButton, self.sizerFlag),(self.runButton, self.sizerFlag),
             (self.stopButton, self.sizerFlag),
        ...
        self.Bind(wx.EVT_BUTTON, self.RunRouteLine, self.runButton)
        self.Bind(wx.EVT_BUTTON, self.StopRun, self.stopButton)
    ...
    def Calibrate(self, event):
        if self.calibratButton.GetToggle():
            ...
            self.calibrateDialog.workCoordinateRotatorTextEntry.SetValue("{}".format(self.workCoordinate))
    ...
    def RunRouteLine(self, event):
        if self.runButton.GetToggle():
            self.runRoutLineDialog.Show()
        else:
            self.runRoutLineDialog.Hide()

    # when stop button pressed stop all running process
    def StopRun(self, event):
        try:
            logger.info(msg="try to stop thread")
            # first check any run thread is alive
            if self.circleDialog.runThread.isAlive():
                # if alive, raise StopError
                _async_raise(self.circleDialog.runThread.ident, StopError)
            if self.findFocusPlaneDialog.startThread.isAlive():
                _async_raise(self.findFocusPlaneDialog.startThread.ident, StopError)
            if self.zigZagDialog.startThread.isAlive():
                _async_raise(self.zigZagDialog.startThread.ident, StopError)
            if self.runRoutLineDialog.startThread.isAlive():
                _async_raise(self.runRoutLineDialog.startThread.ident, StopError)
        except Exception as exception:
            logger.error(msg="fail to stop, because:{}".format(exception))
            wx.MessageDialog.ShowModal(wx.MessageDialog(parent=None, message="failed Stop because error:\n{}"
    ...
    class CircleDialog(InputDialog):
        ...
        def __init__(self, parent):
            ...
            self.sendWaveDataThread = threading.Thread()
        ...
        def ViewOrRun(self, event):
            ...
            if (rMin != self.rMin) \
                ...
                # # set controller
                # try:
                #     # clear wave table 1 and 2 for x and y axis
                #     self.parent.gcs.WCL(1)
                #     self.parent.gcs.WCL(2)
                #     # wirte wave points
                #     pitools.writewavepoints(self.parent.gcs, 1, self.xList.tolist(), 100)
                #     pitools.writewavepoints(self.parent.gcs, 2, self.yList.tolist(), 100)
                #     dataNumber = self.parent.gcs.qWAV(1, 1)[1][1]
                #     print(self.parent.gcs.qGWD(1, 1, dataNumber))
                #     while self.parent.gcs.bufstate is not True:
                #         time.sleep(1)
                #         pass
                #     x = self.parent.gcs.bufdata
                #     # Get wave table 2 data for y
                #     dataNumber = self.parent.gcs.qWAV(2, 1)[2][1]
                #     print(self.parent.gcs.qGWD(2, 1, dataNumber))
                #     while self.parent.gcs.bufstate is not True:
                #         time.sleep(1)
                #         pass
                #     y = self.parent.gcs.bufdata
                # except GCSError as gcserror:
                #     ShowGCSErrorMessage(gcserror)
                #     self.viewCircleButton.Enable()
                #     self.runButton.Enable()
            ...
            if event.GetEventObject() == self.runButton:
                self.runThread = threading.Thread(target=self.RunThread, name='circleThread',
                                                  args=(rMin, rMax, distance, numberOfTurns))
        ...
        def RunThread(self, rMin, rMax, distance, numberOfTurns):
            try:
                ...
                logger.info("start run circle thread, parameter:\n"
                            ...
                            "startPos:{}, WTR:{}, style:{}".format(zStepSize, zStepNumber, colSpace, colNumber,
                                                                   rowSpace,
                ...
                if style == 0:
                    ...
                    for i in range(0, colNumber):
                        for j in range(0, rowNumber):
                            ...
                            if j < rowNumber - 1:
                                self.self.sendWaveDataThread = threading.Thread(target=self.SendWaveData,
                                                                      name='sendWaveDataThread', args=(i, j + 1,
                                                                                                      colSpace,
                                                                                                      rowSpace,
                                                                                                      rowNumber,
                                                                                                      startPos))
                            else:
                                self.sendWaveDataThread = threading.Thread(target=self.SendWaveData,
                                                                      name='sendWaveDataThread', args=(i + 1, 0,
                                                                                                       colSpace,
                                                                                                       rowSpace,
                                                                                                       rowNumber,
                                                                                                       startPos))
                            self.sendWaveDataThread.start()
                            # set start position
                            for k in range(0, zStepNumber):
                                ...
                                self.parent.gcs.MOV({"X": x,
                                                     "Y": y,
                                                     "Z": z})
                                pitools.waitontarget(self.parent.gcs, timeout=600)
                    ...
                    if self.focusPlan is not self.parent.focusPlan or \
                        ...
                        self.WTR = WTR
                    # only when WTR changed need recalc ramp
                    # if self.WTR is not WTR:
                    #     # add ramp list
                    #     velocity = self.distance / (WTR * 0.001)
                    #     # accerleration is 1mm / s ^ 2
                    #     t_ramp = np.arange(0, velocity / 40 , 0.001 * WTR)
                    #     self.yRamp = 0.5 * 40 * t_ramp ** 2
                    #     self.yRamp = self.yRamp - max(self.yRamp)
                    #     self.yRamp = self.yRamp[0:len(self.yRamp)-1]
                    #     # DON't need send ramp to wave tabel 5. it will be done later when add offset
                    #     # save setting
                    #     self.WTR = WTR
                    # save start points two run
                    i = 0
                    j = 0
                    ...
                    for i in range(0, colNumber):
                        for j in range(0, rowNumber):
                            ...
                            pitools.waitontarget(self.parent.gcs, timeout=600)
                            logger.info("move to the start point, x:{}, y:{}, z:{}".format(x, y, z))
                            # only if ramp lenth larger than 0 will do the ramp
                            # if len(self.yRamp) > 1 :
                            #     self.parent.gcs.WTR(2,10,1)
                            #     self.parent.gcs.WGC({2: 1})
                            #     self.parent.gcs.WSL({2: 5})
                            #     # start ramp
                            #     self.parent.gcs.WGO({2: 1})
                            #     # wait untill finished
                            #     while True in self.parent.gcs.qWGO(2).values():
                            #         pass
                            #     self.parent.gcs.WGC({2: zStepNumber})
                            #     sefl.parent.gcs.WTR(2,WTR,1)
                            ###### start run spiral immediately
                            ### connect wave tables.
                            # when even rounds connect table 1 and 2. when odds rounds connect table 3 and 4
                ...
                t2 = time.time()
                self.statusBar.SetStatusText("finish. process spend {}s".format(round(t2-t1, 2)))
                logger.info("successfully run circles process")
            except StopError:
                self.parent.setLaserDialog.Pause()
                self.statusBar.SetStatusText("STOP!!")
                logger.error("circle procee has been stopped")
                self.parent.gcs.HLT(axes=["X", "Y", "Z", "U", "V", "W"], noraise=True)
                self.parent.gcs.WGO({1: 0, 2: 0})
                wx.MessageDialog.ShowModal(wx.MessageDialog(parent=None, message='Stop!:\n', style=wx.OK))
            except Exception as exception:
                self.parent.setLaserDialog.PowerOff()
        ...
        # def SendZAxisWaveData(self, k):
        #     ### Send Z Axis Wave Data method would just perfomed as send offset z axis will moving
        #     # add offset
        #     try:
        #         self.parent.gcs.WAV_LIN(table=k%2 + 5, firstpoint=0, numpoints=self.xList.shape[0] / 10, append="X",
        #                                 speedupdown=10, amplitude=self.zStepSize,
        #                                 offset=self.parent.focusPlan["Z"] + k * self.zStepSize,
        #                                 seglength=self.xList.shape[0] / 10)
        #     except GCSError as gcserror:
        #         ShowGCSErrorMessage(gcserror)
    ...
    class CalibrateDialog(InputDialog):
        ...
        def __init__(self,parent):
            ...
            self.workCoordinateRotatorText = wx.StaticText(parent=self,label="Coordinate rotate:")
            self.workCoordinateRotatorTextEntry = wx.TextCtrl(parent=self,value="{} ".
                                                              format(self.parent.workCoordinate),
                                                              style=wx.TE_PROCESS_ENTER)
            ...
            self.velocityText = wx.StaticText(parent=self, label="speed: ")
            self.veloctyTextEntry = wx.TextCtrl(parent=self, value="{}".format(self.parent.gcs.qVLS()), size=(70,26), style=wx.TE_PROCESS_ENTER)
            self.velocityUnit = wx.StaticText(parent=self, label="mm/s", pos=(200,195))
            ...
            self.gridSizer.AddMany([(0,0,0,0),(0,0,0,0),
                                    ...
                                    (self.workCoordinateRotatorText, self.staticTextSizeFlag),
                                    (self.workCoordinateRotatorTextEntry, self.textEntrySizerFlag),
                                    ...
                                    (self.velocityText,self.staticTextSizeFlag),
                                    (self.veloctyTextEntry,self.textEntrySizerFlag),
                                    (self.loadCalibrateFileText,self.staticTextSizeFlag),
                                    (self.loadCalibrateFileButton,self.textEntrySizerFlag),
                                    ] )
            self.SetSizer(self.gridSizer)
            self.staticLine.SetPosition((0,272))
            self.staticLine.SetSize((350,2))
            self.cancelButton.SetPosition((245, 279))
            self.OKButton.SetSize(88,30)
            self.OKButton.SetPosition((146,279))
            # Bind Event
                ...
                if "CameraCalibrate.pkl" in self.fileName:
                    with open(self.fileName,"rb") as file:
                        try:
                            ...
                            workCoordinateRotator = pickle.load(file)
                            self.workCoordinateRotator = workCoordinateRotator
        ...
        def SetCalibration(self,event):
            ...
            workCordinateRotator = float(self.workCoordinateRotatorTextEntry.GetValue())
            velocity = float(self.veloctyTextEntry.GetValue())
            self.parent.expend = expend
            self.parent.rotator = rotator
            self.parent.pos = pos
            self.parent.workCoordinate = workCordinateRotator
            self.parent.gcs.VLS(velocity)
            # set work coordinate
            try:
                # first must disable the current work coordinate
                self.parent.gcs.KEN(0)
                # define new one. And only roatate XY plane
                self.parent.gcs.KSD(self.parent.username, 'W', self.parent.workCoordinate)
                # enable work coordinate
                self.parent.gcs.KEN(self.parent.username)
            except GCSError as gcserror:
                ShowGCSErrorMessage(gcserror)
            print(self.parent.gcs.qKLS(self.parent.username,'POS'))
            # show save dialog
            if wx.MessageDialog.ShowModal(
                    wx.MessageDialog(parent=None,
                ...
                try:
                    with open(self.fileName, "wb+") as file:
                        ...
                        pickle.dump(workCordinateRotator,file,4)
                        logger.info("sucessfully save calibrate file:{}\n"
                                    "expand:{}, rotater:{}, pos:{}, workCoordinate:{}"
                                    .format(self.fileName, expend, rotator, pos, workCordinateRotator))
    ...
    class AutoFindFocuseDialog(InputDialog):
        ...
        def __init__(self, parent):
            ...
            self.SetSizer(self.gridSizer)
            # add thread
            self.startThread = threading.Thread(target=self.FindFocusePlan)
            # Bind Event
        ...
        def FindFocusePlan(self):
            try:
                ...
                self.parent.setLaserDialog.Pause()
                logger.info("find focuse plan has been stopped")
                self.parent.gcs.HLT(axes=["X", "Y", "Z", "U", "V", "W"], noraise=True)
                wx.MessageDialog.ShowModal(wx.MessageDialog(parent=None, message='Stop!:\n', style=wx.OK))
            except Exception as ex:
                self.parent.setLaserDialog.PowerOff()
    ...
    class ZigZagDialog(InputDialog):
        ...
        def __init__(self, parent):
            ...
            self.SetSizer(self.gridSizer)
            # add thread
            self.startThread = threading.Thread(target=self.ZigZag)
            # Bind Event
            self.Bind(wx.EVT_BUTTON, self.StarThread, self.startButton)
        ...
        def ZigZag(self):
            try:
                ...
                self.parent.setLaserDialog.Pause()
                logger.info("ZigZag has been stopped")
                self.parent.gcs.HLT(axes=["X", "Y", "Z", "U", "V", "W"], noraise=True)
                wx.MessageDialog.ShowModal(wx.MessageDialog(parent=None, message='Stop!:\n', style=wx.OK))
            except Exception as ex:
    ...
    class SetLaserDialog(InputDialog):
        ...
        def SetLaserParameter(self):
            ...
            self.burstLength = int(float(self.burstLengthTextEntry.GetValue()))
            checkFlage = True
    ...
    class RunRouteLineDialog(InputDialog):

        def __init__(self, parent):
            self.parent = parent
            super(MainCameraPanel.RunRouteLineDialog, self).__init__(parent=self.parent, title="Run route",
                                                                  size=(320, 240))
            # initialize property
            self.fileName = "None"
            self.routList = []
            # add items
            self.loadRouteFileText = wx.StaticText(parent=self, label="Load from file:",size=(150, 25),pos=(10,10))
            self.loadRouteFileName = wx.StaticText(parent=self, label="{}".format(self.fileName), size=(250, 25), pos=(20,30),
                                                           style=wx.ST_ELLIPSIZE_START)
            self.loadRouteFileButton = wx.Button(parent=self, label="Load", size=(50, 26),pos=(180,60))
            self.styleText = wx.StaticText(parent=self, label="Sample style: ",pos=(10,90))
            self.styleChoice = wx.Choice(parent=self, choices=["Line", "Spot"], size=(70, 26),pos=(180,90),
                                         style=wx.TAB_TRAVERSAL | wx.TE_PROCESS_ENTER)
            self.styleChoice.SetSelection(0)
            self.speedText = wx.StaticText(parent=self, label="speed: ", pos=(10,120))
            self.speedTextEntry = wx.TextCtrl(parent=self, value="10", size=(70,26), pos=(180,120), style=wx.TE_PROCESS_ENTER)
            self.speedUnit = wx.StaticText(parent=self, label="mm/s",pos=(260,125))
            self.staticLine = wx.StaticLine(parent=self,size=(300,2),pos=(0,150))
            self.RunButton = wx.Button(parent=self, label="Run",size=(88,30), pos=(180,160))
            # add thread
            self.startThread = threading.Thread(target=self.RunRoute, daemon=True)
            # Bind Event
            self.Bind(wx.EVT_BUTTON, self.LoadFile, self.loadRouteFileButton)
            self.Bind(wx.EVT_BUTTON, self.StarThread, self.RunButton)

        def StarThread(self, event):
            self.startThread = threading.Thread(target=self.RunRoute)
            self.startThread.start()
        ...
        def RunRoute(self):
            try:
                ...
                speed = round(float(self.speedTextEntry.GetValue()), 3)
                style = self.styleChoice.GetSelection()
                pos = self.parent.gcs.qPOS(["X", "Y", "Z"])
                # check routList have point
                if not self.routList.__len__():
                    raise Exception("Please load route first")
                if not (0 < speed <= 10):
                    raise Exception("speed out of limit, range (0, 10]")
                # if set laser dialog haven't show, show it
                if not self.parent.setLaserButton.GetToggle():
                ...
                self.parent.gcs.VLS(speed)
                # start run route
                for i in range(self.routList.__len__()):
                    tmpPoint = self.routList[i]
                    self.parent.gcs.MOV({"X": tmpPoint[0] + pos["X"], "Y": tmpPoint[1] + pos["Y"],
                                         "Z": tmpPoint[2] + pos["Z"]})
                    time.sleep(0.2)
                    if style == 0:
                        # laser darw line
                        ...
                        # set laser parameter
                        self.parent.setLaserDialog.attenuatorTextEntry.SetValue("{}".format(tmpPoint[4]))
                        self.parent.setLaserDialog.burstLengthTextEntry.SetValue("{}".format(tmpPoint[5]))
                        self.parent.setLaserDialog.SetLaserParameter()
                        # set burst mode to trigger, if it is not
                        if self.parent.setLaserDialog.GetModeContinueOrTrigger() != "Trigger":
                            self.parent.setLaserDialog.SetModeTrigger()
                        ...
                        while self.parent.setLaserDialog.GetBurstToGo() != "0.0":
                            time.sleep(0.01)
                    # wait until stop
                    while True in self.parent.gcs.IsMoving(axes=["X", "Y", "Z", "U", "V", "W"]).values():
                        time.sleep(0.2)
                t2 = time.time()
                self.parent.setLaserDialog.PowerOff()
                wx.MessageDialog.ShowModal(wx.MessageDialog(parent=None,
                                                            message="Successfully finished. Process using {}s:\n"
                                                            .format(round(t2 - t1,2)), style=wx.OK))
                logger.info("Successfully finish run route")
            except StopError:
                self.parent.setLaserDialog.Pause()
                logger.info("RunRoute has been stopped")
                self.parent.gcs.HLT(axes=["X", "Y", "Z", "U", "V", "W"], noraise=True)
                wx.MessageDialog.ShowModal(wx.MessageDialog(parent=None, message='Stop!:\n', style=wx.OK))
            except Exception as ex:
                if ex.args != "speed out of limit, range (0, 10]":
                    self.parent.setLaserDialog.PowerOff()
                logger.error("failed do run route because:{}".format(ex))
                ShowErrorMessage("failed do run route because:{}".format(ex))
            finally:
                self.parent.laserSpotTriggerButton.Enable()
                self.RunButton.Enable()
                self.parent.circleDialog.runButton.Enable()
        ...
        def LoadFile(self, event):
            self.fileName = wx.FileSelector(message="Load File", default_path=".\\data\\",
                                            ...
                                            flags=wx.FD_OPEN)
            if self.fileName.strip():
                self.loadRouteFileName.SetLabel("{}".format(self.fileName))
                logger.info("load {}".format(self.fileName))
            with open(self.fileName, "rb") as file:
                try:
                    # clear current list
                    ...
                    i = 0
                    for i in range(tmpList.__len__()):
                        if not self.Str2Float(tmpList[i]).__len__() > 6:
                            self.routList.append(self.Str2Float(tmpList[i]))
                        else:
                            self.routList.clear()
                            raise Exception("route file error")
                    wx.MessageDialog.ShowModal(
                        wx.MessageDialog(parent=None,
                                         message="successfully load route file:"
                                                 "\n{}".format(self.fileName),
                                         style=wx.OK))
                    logger.info("successfully load route file:{}".format(self.fileName))
                except Exception as ex:
                    logger.error("failed load route file because:{}".format(ex))
                    wx.MessageDialog.ShowModal(
                        wx.MessageDialog(parent=None,
                                         message="exception accured in reading :"
                                                 "\n{}\nbecause:{}".format(self.fileName, ex),
                                         style=wx.OK))

    #   transform string to int. Return a list
        def Str2Float(self,bytes):
            intList = map(float, bytes.decode().split(','))
            return list(intList)
